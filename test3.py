...
def run_predictions(model, X_test):
    ## Evaluate the predictions
    predictions = model.predict(X_test)
    return predictions

def mean_abs_error(predictions, y_test):
    ## Mean Absolute error
    ## https://scikit-learn.org/stable/modules/model_evaluation.html
    return print("Mean Absolute Error : " + str(mean_absolute_error(predictions, y_test)))

def model_accuracy(predictions, y_test):
    ## Model Accuracy i.e how correct is the classifier?
    accuracy = accuracy_score(y_test, predictions)
    return print('Accuracy of Model is : %.2f%%' % (accuracy * 100.0))
